  ...
  return (
    <div
      ...
      style={{
        ...
        borderLeft: '1px solid rgba(197,198,210,0.2)',
      }}
    >
      <div className="flex justify-between items-center mb-1">
        ...
        <span className="text-xs font-bold" style={{ color: '#ba1a1a' }}>
          {'★'.repeat(review.rating)}{'☆'.repeat(5 - review.rating)}
        </span>
      </div>
      <p className="text-xs mb-3 line-clamp-2" style={{ color: '#444650' }}>
        "{review.content}"
      </p>
      <button
        onClick={onAiReply}
        className="flex items-center gap-1.5 text-xs font-bold cursor-pointer transition-all hover:opacity-80 px-3 py-1.5 rounded-lg"
...
export default function NeedsAttention() {
  ...
  const critical = MOCK_REVIEWS
    .filter((r) => r.status === 'pending' && r.rating <= 3 && !repliedIds.has(r.id))
    .slice(0, 3);
    ...
    <>
      ...
      >
        <div className="flex items-center gap-2 mb-6">
          <span className="material-symbols-outlined text-[20px]" style={{ color: '#ba1a1a' }}>
            priority_high
          </span>
        ...
        <div className="space-y-3 flex-1">
          {critical.map((r) => (
            <ReviewItem
              key={r.id}
              review={r}
              onAiReply={() => setAiTarget(r)}
            />
          ))}
          {critical.length === 0 && (
            <p className="text-sm text-center py-4" style={{ color: '#444650' }}>
              כל הביקורות טופלו ✓
            </p>
          )}
        </div>
        ...
        <button
          ...
          className="w-full mt-5 py-2.5 rounded-lg text-sm font-semibold transition-colors hover:opacity-90 cursor-pointer border"
          style={{ borderColor: 'rgba(0,17,58,0.2)', color: '#00113a', backgroundColor: 'transparent' }}
        >
          צפה בכל {MOCK_REVIEWS.filter((r) => r.status === 'pending').length} הביקורות
        </button>
      </div>
